...
async fn test_router_with_tracing() {
    ...
    let _log_guard = logging::init_logging(
        logging::LoggingConfig {
            ...
            log_targets: Some(vec!["mesh".to_string()]),
        },
        Some(trace_config),
    );
    ...
    let _span = info_span!(target: "mesh::otel-trace", "test_router_with_tracing");
...
async fn test_grpc_trace_context_injection() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        let span = info_span!(target: "mesh::otel-trace", "test_grpc_span");
