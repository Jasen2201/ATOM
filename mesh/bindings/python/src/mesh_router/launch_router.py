...
from mesh_router.mini_lb import MiniLoadBalancer
from mesh_router.router_args import RouterArgs

logger = logging.getLogger("router")

try:
    from mesh_router.router import Router
except ImportError:
    Router = None
    logger.warning(
...
def launch_router(args: argparse.Namespace) -> Optional[Router]:
    """
    Launch the ATOM Mesh router with the configuration from parsed arguments.
...
def parse_router_args(args: List[str]) -> RouterArgs:
    ...
    parser = argparse.ArgumentParser(
        description="""ATOM Mesh Router - High-performance request distribution across worker nodes
...
Examples:
  # Regular mode
  python -m mesh_router.launch_router --worker-urls http://worker1:8000 http://worker2:8000

  # PD disaggregated mode with same policy for both
  python -m mesh_router.launch_router --pd-disaggregation \\
