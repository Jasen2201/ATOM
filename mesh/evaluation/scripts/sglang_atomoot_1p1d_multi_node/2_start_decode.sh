...
# ---- Mooncake config (must match prefill node's config) ----
PREFILL_HANDSHAKE_IP="${PREFILL_HANDSHAKE_IP:?ERROR: Set PREFILL_HANDSHAKE_IP to the prefill node management IP}"
PREFILL_PORT="${PREFILL_PORT:-8010}"
MOONCAKE_CONFIG_FILE="${MOONCAKE_CONFIG_FILE:-mooncake_${MOONCAKE_PROTOCOL}.json}"
...
MCEOF
    ...
    echo "[config] Mooncake config found at ${MOONCAKE_CONFIG_PATH}"
fi
