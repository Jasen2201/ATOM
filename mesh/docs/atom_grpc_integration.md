...
### Request and Response Data at Each Boundary

```mermaid
flowchart LR
    subgraph Client_to_Mesh["Client → Mesh"]
        IN1["POST /v1/chat/completions<br/>{model, messages[], tools[], stream}"]
    end

    subgraph Mesh_to_ATOM["Mesh → ATOM (gRPC Request)"]
        IN2["GenerateRequest<br/>{request_id, tokenized.input_ids[],<br/>sampling_params, stream}"]
    end

    subgraph ATOM_to_Mesh["ATOM → Mesh (gRPC Response Stream)"]
        OUT1["0..N × Chunk<br/>{token_ids[], prompt_tokens,<br/>completion_tokens, cached_tokens}"]
        OUT2["1 × Complete<br/>{output_ids[], finish_reason,<br/>prompt_tokens, completion_tokens}"]
        OUT3["or 1 × Error<br/>{message, http_status_code}"]
    end

    subgraph Mesh_to_Client["Mesh → Client"]
        OUT4["SSE stream<br/>data: {choices:[{delta:{content}}]}<br/>...<br/>data: {choices:[{finish_reason:stop}]}<br/>data: [DONE]"]
    end
    ...
    style Client_to_Mesh fill:#f3e5f5,stroke:#9C27B0
    style Mesh_to_ATOM fill:#e8f4f8,stroke:#2196F3
    style ATOM_to_Mesh fill:#fff3e0,stroke:#FF9800
    style Mesh_to_Client fill:#e8f5e9,stroke:#4CAF50
```

**Summary of what crosses each boundary:**

| Boundary | Direction | Format | Key Fields |
|----------|-----------|--------|------------|
| Client → Mesh | HTTP POST | OpenAI JSON | `messages[]`, `tools[]`, `model`, `stream`, `temperature`, ... |
| Mesh → ATOM | gRPC | `GenerateRequest` proto | `request_id`, `tokenized.input_ids[]`, `sampling_params`, `stream` |
| ATOM → Mesh (streaming) | gRPC stream | `GenerateResponse::Chunk` | `token_ids[]` (incremental), `prompt_tokens`, `completion_tokens` (cumulative) |
| ATOM → Mesh (final) | gRPC stream | `GenerateResponse::Complete` | `output_ids[]` (all tokens), `finish_reason` (`"stop"`/`"length"`/`"abort"`), token counts |
| ATOM → Mesh (error) | gRPC stream | `GenerateResponse::Error` | `message`, `http_status_code`, `details` |
| Mesh → Client | HTTP SSE | OpenAI JSON chunks | `choices[].delta.content`, `choices[].finish_reason`, `usage` |

### Connection Mode Auto-Detection

Mesh automatically selects the connection mode based on the worker URL prefix — no additional configuration needed:

```mermaid
flowchart LR
    URL["Worker URL"] --> Check{"URL prefix?"}
    Check -- "http://" --> HTTP["HTTP Pass-through Mode<br/>Mesh = Reverse Proxy"]
    Check -- "grpc://" --> GRPC["gRPC Pipeline Mode<br/>Mesh = Full-stack Processing"]
    
    HTTP --> ENGINE_HTTP["Engine must implement<br/>full HTTP API"]
    GRPC --> ENGINE_GRPC["Engine only needs<br/>5 gRPC RPCs"]
