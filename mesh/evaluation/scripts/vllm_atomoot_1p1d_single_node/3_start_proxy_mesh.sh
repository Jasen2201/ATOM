#!/usr/bin/env bash
# =============================================================================
# Script 3: Start PD Proxy - Single Node 1P1D (atom-mesh)
# Routes requests through Prefill -> Decode on localhost
# Uses atom-mesh (mesh) for PD disaggregated routing
# =============================================================================
set -euo pipefail
...
# atom-mesh binary: prefer /usr/local/bin/mesh (container), fallback to project build
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
if [[ -z "${MESH_BIN:-}" ]]; then
    if command -v mesh >/dev/null 2>&1; then
        MESH_BIN="$(command -v mesh)"
    else
...
echo ""
echo "============================================================"
echo "  PD Proxy - Single Node 1P1D (atom-mesh)"
echo "============================================================"
